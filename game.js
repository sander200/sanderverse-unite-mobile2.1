...
(function () {
  ...
  class AssetLoader {
    ...
    prepare(id) {
      ...
      if (id.indexOf("sheet_") === 0) { this.clean[id] = img; return img; }
  ...
  class MapBuffer {
    ...
    rebuild(img) {
      ...
      // caminhos transitáveis
      g.lineCap = "round";
      g.strokeStyle = "rgba(40,36,28,0.55)";
      g.lineWidth = 78;
      walkableLanes.forEach(function (l) { g.beginPath(); g.moveTo(l.x1, l.y1); g.lineTo(l.x2, l.y2); g.stroke(); });
      g.strokeStyle = "rgba(150,136,96,0.45)";
      g.lineWidth = 52;
      walkableLanes.forEach(function (l) { g.beginPath(); g.moveTo(l.x1, l.y1); g.lineTo(l.x2, l.y2); g.stroke(); });
  ...
  class SanderPlayer {
    ...
    update(dt) {
      ...
      if (!this.isChanneling) {
        ...
        constrainToPath(this, this.isDashing ? 84 : 56);
        resolveSolids(this);
        constrainToPath(this, this.isDashing ? 84 : 56);
      }
    ...
    render(context, camX, camY) {
      ...
      let drawn = false;
      if (sheet) drawn = drawSheetFrame(context, sheet, frameNum, 4, 4, sx - 36, sy - 70, 72, 108, this.facingLeft);
      if (!drawn) drawn = drawUnit(context, assets.get(this.hero.portrait), sx - 30, sy - 62, 60, 92, this.facingLeft);
      if (!drawn) {
        context.beginPath(); context.arc(sx, sy, this.radius, 0, Math.PI * 2);
      ...
      context.fillText("NV" + this.level + (this.auraCount ? "  ⚡" + this.auraCount : ""), sx, sy + 48);
      context.restore();
    }
  }
  ...
  class EnemyBird {
    ...
    render(context, camX, camY) {
      ...
      let drawn = false;
      if (sheet) drawn = drawSheetFrame(context, sheet, frameNum, 4, 4, sx - 34, sy - 66, 68, 100, this.facingLeft);
      if (!drawn) drawn = drawUnit(context, assets.get(this.portraitKey), sx - 28, sy - 56, 56, 84, this.facingLeft);
      if (!drawn) { context.beginPath(); context.arc(sx, sy, this.radius, 0, Math.PI * 2); context.fillStyle = this.color; context.fill(); }
      context.fillStyle = "rgba(0,0,0,0.5)"; context.fillRect(sx - 24, sy - 62, 48, 5);
      context.fillStyle = "#ff3366"; context.fillRect(sx - 24, sy - 62, Math.max(0, this.hp / this.maxHp) * 48, 5);
      context.fillStyle = "#fff"; context.font = "10px sans-serif"; context.textAlign = "center";
  ...
  class NemesisBoss {
    ...
    update(dt, pl) {
      ...
      if (this.mode === "chase") {
        ...
        this.x += dx / d * this.speed * 0.7 * dt; this.y += dy / d * this.speed * 0.7 * dt;
      }
      if (pl.x < this.x) this.facingLeft = true; else this.facingLeft = false;
      this.x = Math.max(80, Math.min(WORLD_WIDTH - 80, this.x));
      this.y = Math.max(80, Math.min(WORLD_HEIGHT - 80, this.y));
      constrainToPath(this, this.omega ? 96 : 78);
      resolveSolids(this);
      constrainToPath(this, this.omega ? 96 : 78);
      if (this.atkCd <= 0) {
  ...
  function doFlash() {
    ...
    player.x += Math.cos(dash) * 140; player.y += Math.sin(dash) * 140;
  ...
  function doTeleport() {
    ...
    player.x += Math.cos(dash) * 280; player.y += Math.sin(dash) * 280;
    player.x = Math.max(player.radius, Math.min(WORLD_WIDTH - player.radius, player.x));
    player.y = Math.max(player.radius, Math.min(WORLD_HEIGHT - player.radius, player.y));
    player.invuln = 0.45;
    sound.teleport();
